						  ...
						  void *context)
{
	...
	if (fw != NULL) {
		...
		new_version = (fw->data[0x0016fffc] << 8) | fw->data[0x0016fffd];
		ret = m6mo_s_power(sd, 1);  /*camera power on*/
		if (ret) {
			pr_err("%s():power fail\n", __func__);
			goto err_firmware_fail;
		}
		...
		if(m6mo_get_checksum(sd, &chsum) == 0) {
			...
			if (ret)  goto err_firmware_fail; /*other error ? like i2c*/

			ret = m6mo_i2c_read_category_16bit(sd, VER_FIRMWARE_MINOR, &value);
			if(ret) goto err_firmware_fail;
		...
		printk("firmware old version = 0x%04x, new version = 0x%04x\n",  
			state->firmware_version, new_version);

		if (new_version != state->firmware_version) {
			/*shoud reset camera to let it in flash writer mode*/
			m6mo_s_power(sd, 0);
			ret = m6mo_s_power(sd, 1);
			if(ret) goto err_firmware_fail;
			msleep(100);

			/*now begin to update firmware, should get a wake lock*/
...
err_firmware_fail:
	if (state->power) m6mo_s_power(sd, 0);
	m6mo_set_firmware_status(state, FIRMWARE_LOADED_FAIL);
}
...
static int m6mo_runtime_suspend(struct device *dev)
{
...
static int m6mo_runtime_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct m6mo_state *state = to_state(sd);
...
static const struct dev_pm_ops m6mo_pm_ops = {
	.runtime_suspend = m6mo_runtime_suspend,
	.runtime_resume = m6mo_runtime_resume,
};
